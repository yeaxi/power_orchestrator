...
async def test_service_registration_exposes_only_safe_handlers() -> None:
    ...
    registered = {call.args[1]: call.args[2] for call in hass.services.async_register.call_args_list}
    ...
    call = SimpleNamespace(data={"device_id": "d1", "source": "test"}, context=SimpleNamespace(user_id="u", id="c"))
    await registered["request_stop"](call)
    await registered["clear_quarantine"](call)
    await registered["set_execution_mode"](SimpleNamespace(data={"execution_mode": "observe", "confirm_live": False}))
    coordinator.async_force_evaluate.assert_awaited_once()
    coordinator.async_set_mode.assert_awaited_once_with(MODE_AUTO)
    coordinator.async_request_stop.assert_awaited_once_with("d1", source="test", actor_id="u", context_id="c")
    coordinator.async_clear_quarantine.assert_awaited_once_with("d1", source="test", actor_id="u", context_id="c")
...
def test_repair_helpers_and_unregister_are_bounded() -> None:
    entry = SimpleNamespace(runtime_data=SimpleNamespace(coordinator=SimpleNamespace(_quarantined={"d1"})))
...
def test_repair_issue_sync_tracks_faults_and_removes_stale_persistent_issues() -> None:
    coordinator = SimpleNamespace(
        data={"faulted_devices": ["d1"], "quarantined_devices": ["d1"]}
    )
    runtime = SimpleNamespace(coordinator=coordinator)
    entry = SimpleNamespace(entry_id="entry-1", runtime_data=runtime)
    hass = _hass_with_services(runtime=runtime)
