...
def test_threshold_form_has_no_generation_or_start_fields() -> None:
    flow = PowerOrchestratorConfigFlow()
    schema_names = {getattr(key, "schema", key) for key in flow._load_monitoring_form()["data_schema"].schema}
...
def test_device_form_contains_only_shedding_fields() -> None:
    flow = PowerOrchestratorConfigFlow()
    schema_names = {getattr(key, "schema", key) for key in flow._device_config_form({})["data_schema"].schema}
